...
from ..agent.agent_mujoco import Agent, quaternion_to_matrix
...
class SpotmicroEnv(gym.Env):
    ...
    to create an interface between Stable Baselines3 and mujoco:
    ...
    - close: Closes the mujoco simulation, saves the state if a destination path is provided.
    ...
    **Private methods**
    - _step_simulation: accepts an action and returns an observation. This is done by the Agent class:
                        apply_action(action) -> mujoco.mj_step -> sync_state()
...
"""
    ...
    def __init__(self, device: Device, config: Config, reward_fn: callable, reward_state, use_gui=False, tracker_on=False, dest_save_file=None, src_save_file=None, writer=None,
        ...
        # Initialize MuJoCo - the model is loaded in the Agent class
        self._model = None
        self._data = None
        self._agent = Agent(self, device, config, self._ACT_SPACE_SIZE)
        
        # Get model and data from agent
        self._model = self._agent._model
        self._data = self._agent._data
        
        # Launch MuJoCo viewer if requested
        self._viewer = None
        if use_gui:
            self._viewer = mujoco.viewer.launch_passive(self._model, self._data)
        
        # MuJoCo doesn't need terrain in the same way - floor is in the XML
    ...
    def reset(self, seed: int | None = None, options: dict | None = None) -> tuple[np.ndarray, dict]:
        """
        Gymnasium reset: start a new episode, set counters, reset agent,
        and return the initial observation and info dict.\n

        **More specifically**: \n
        - Resets Agent using reset() method
        - steps the mujoco simulation a few times until the bot is stable in the initial state
    ...
    def _step_simulation(self, action: np.ndarray) -> np.ndarray:
        """
        Private method that calls the API to execute the given action in MuJoCo.
        It should synchronize the state of the agent in the simulation with the state recorded here!
    ...
    def _get_gravity_vector(self) -> np.ndarray:
        ...
        rot_matrix = quaternion_to_matrix(self._agent.state.base_orientation)
